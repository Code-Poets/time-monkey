...
class UserDetailTests(TestCase):
    ...
    def test_user_detail_view_should_update_user_on_post(self):
        old_phone_number = ''.join(random.choices(string.digits, k=constants.PHONE_NUMBER_MIN_LENGTH))
        ...
        current_phone_number = self.user.phone_number
        self.assertEqual(response.status_code, 302)
        self.assertEqual(current_phone_number, old_phone_number)
    ...
    def test_user_detail_view_should_not_update_user_on_post_if_form_is_invalid(self):
        ...
        invalid_phone_number = ''.join(random.choices(string.digits, k=constants.PHONE_NUMBER_MIN_LENGTH - 1))
...
class UserUpdateTests(TestCase):
    ...
    def test_user_update_view_should_update_user_on_post(self):
        ...
        request.user = self.user
        response = self.client.post(
            path=reverse('custom-user-update', args=(self.user.pk,)),
            data={
                'phone_number': old_phone_number,
            },
        )
    ...
    def test_user_update_view_should_not_update_user_on_post_if_form_is_invalid(self):
        phone_number_before_request = self.user.phone_number
        old_phone_number = ''.join(random.choices(string.digits, k=constants.PHONE_NUMBER_MIN_LENGTH - 1))
        request = APIRequestFactory().post(
            path=reverse('custom-user-update', args=(self.user.pk,)),
            data={'phone_number': old_phone_number, },
        )
        request.user = self.user
        response = views.UserUpdate.as_view()(request, pk=self.user.pk)
        self.user.refresh_from_db()
