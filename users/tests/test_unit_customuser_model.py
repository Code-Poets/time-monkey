...
from users.common.utils import user_full_clean_and_save
...
class TestCustomUserModel(TestCase):
    ...
    def test_user_with_existing_email_full_clean_should_raise_validation_error(self):
        user_full_clean_and_save('testuser@codepoets.it', '', '', '', 'newuserpasswd')
    ...
    def test_user_with_email_without_at_least_one_at_sign_full_clean_should_raise_custom_validation_error(self):
        with self.assertRaises(CustomValidationError) as custom_exception:
            user_full_clean_and_save('wrongtestusercodepoets.it', '', '', '', 'newuserpasswd')
        self.assertEqual(
            ...
            user_full_clean_and_save(
                ...
                ''.join(
                    random.choices(
                        ...
                        k=constants.PHONE_NUMBER_MIN_LENGTH,
                    ),
                ),
                'newuserpasswd',
            )
...
class TestCustomUserModelMethods(TestCase):
    def setUp(self):
        user_full_clean_and_save("testuser@codepoets.it", "testusername", "testuserlastname", "", "testuserpasswd")
    ...
    def test_that_get_absolute_url_method_should_return_absolute_url_with_users_email(self):
        ...
        self.assertEqual(stored_user.get_absolute_url(), f"/users/{email}/")
