...
class TestProjectModelField(BaseModelTestCase):
    ...
    def setUp(self):
        self.manager = create_user_using_full_clean_and_save(
            'manager@codepoets.it',
            '',
            '',
            '',
            'managerpassword',
        )
        self.member = create_user_using_full_clean_and_save(
            'projectmember@codepoets.it',
            '',
            '',
            '',
            'memberpassword',
    ...
    def test_project_model_start_date_field_should_accept_correct_input(self):
        self.field_should_accept_input('start_date', datetime.datetime.now().date())
    ...
    def test_project_model_start_date_field_should_not_accept_non_date_or_datetime_value(self):
        ...
        self.field_should_not_accept_input('start_date', generate_random_string_from_letters_and_digits(MAX_NAME_LENGTH))  # pylint: disable=line-too-long # noqa E501

    def test_project_model_stop_date_field_should_accept_correct_input(self):
        self.field_should_accept_input('stop_date', datetime.datetime.now().date())
