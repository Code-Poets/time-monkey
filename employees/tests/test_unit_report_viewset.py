...
from employees.views import ProjectReportDetail
...
class ProjectReportDetailTests(TestCase):
    ...
    def test_project_report_detail_view_should_display_report_details(self):
        request = APIRequestFactory().get(path=reverse("project-report-detail", args=(self.report.pk,)))
        request.user = self.user
        response = ProjectReportDetail.as_view()(request, pk=self.report.pk)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.report.description)
        self.assertEqual(response.data["serializer"].instance, self.report)

    def test_project_report_detail_view_should_not_be_accessible_for_unauthenticated_users(self):
        request = APIRequestFactory().get(path=reverse("project-report-detail", args=(self.report.pk,)))
        request.user = AnonymousUser()
        response = ProjectReportDetail.as_view()(request, pk=self.report.pk)
        self.assertEqual(response.status_code, 403)

    def test_project_report_detail_view_should_not_render_non_existing_report(self):
        request = APIRequestFactory().get(path=reverse("project-report-detail", args=(999,)))
        request.user = self.user
        response = ProjectReportDetail.as_view()(request, pk=999)
        self.assertEqual(response.status_code, 404)

    def test_project_report_detail_view_should_update_report_on_post(self):
        new_description = "Some other description"
        request = APIRequestFactory().post(
            path=reverse("project-report-detail", args=(self.report.pk,)),
            data={
                "date": datetime.datetime.now().date(),
                "description": new_description,
                "project": self.project,
    ...
    def test_project_report_detail_view_should_not_update_report_on_discard(self):
        new_description = "Some other description"
        request = APIRequestFactory().post(
            path=reverse("project-report-detail", args=(self.report.pk,)),
            data={
