...
class AuthorReportViewTests(TestCase):
    ...
    def test_author_report_list_view_should_display_monthly_hour_sum_in_hour_format(self):
        date = timezone.datetime(year=self.current_time.year, month=self.current_time.month, day=1)
        for i in range(4):
            ReportFactory(
                author=self.user, date=date + timezone.timedelta(days=i), work_hours=timezone.timedelta(hours=8)
            )
...
class ReportCustomListTests(TestCase):
    ...
    def test_custom_report_list_view_should_display_monthly_hour_sum_in_hour_format(self):
        date = timezone.datetime(year=timezone.now().year, month=timezone.now().month, day=1)
        for i in range(3):
            ReportFactory(
                author=self.user, date=date + timezone.timedelta(days=i), work_hours=timezone.timedelta(hours=8)
            )
...
class ProjectReportListTests(TestCase):
    ...
    def test_project_report_list_view_should_display_monthly_hour_sum_in_hour_format(self):
        date = timezone.datetime(year=self.year, month=self.month, day=1)
        for i in range(3):
            ReportFactory(
                author=self.user,
                project=self.project,
                date=date + timezone.timedelta(days=i),
